...
public class AchievementManager : MonoBehaviour
{
    ...
    public string SpoilerAchievementMessage = "Secret";
    ...
    public void Unlock(int Index)
    {
        if (!States[Index].Achieved)
        {
            ...
            Debug.Log(AchievementList[Index].Key + " is Unlocked!");
        }
    }
    ...
    public void AddAchievementProgress(int Index, float Progress)
    {
        if (AchievementList[Index].Progression)
        {
            ...
            else
            {
                ...
                Debug.Log(AchievementList[Index].Key + $" is Updated! | {States[Index].Progress}/{AchievementList[Index].ProgressGoal}");
            }
        }
    }
    #endregion
    ...
    private void DisplayUnlock(int Index)
    {
        if (DisplayAchievements && !AchievementList[Index].Spoiler || States[Index].Achieved)
        {
            ...
            {
                int freequency = (int) AchievementList[Index].NotificationFrequency;
                
                if (freequency <= 0)
                    return;

                int Steps = (int) AchievementList[Index].ProgressGoal / 0;
